...
module.exports = class Snippets {
    ...
    parse() {
        ...
        const snippetObj = {}
        try {
            for (const file of fs.readdirSync(path.resolve(__dirname, 'snippets'))) {
                ...
                const snippets = Object.keys(snippetData).map((e) => ({
                    ...
                    documentation: JSON.stringify(snippetData[e].body, null, 4),
                    insertText: JSON.stringify(snippetData[e].body, null, 4),
    ...
    static async load(connection, documentUri) {
        ...
        if (is_snippets_enabled) {
            ...
            return snippets[path.extname(documentUri).slice(1)] ?? []
        } else return []
    }
}
